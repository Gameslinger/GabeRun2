...
import GameObjects.portalPair.Portal;
import GameObjects.portalPair.PortalPair;
...
public class Draw {
    ...
    public static void portal() {
        for (PortalPair pair : GabeRun2.portals) {
            gs.fillOval(pair.getPortal1().getpX(), pair.getPortal1().getpY(), pair.getPortal1().getWitdh(), pair.getPortal1().getHeight());
            gs.fillOval(pair.getPortal2().getpX(), pair.getPortal2().getpY(), pair.getPortal1().getWitdh(), pair.getPortal1().getHeight());
        }
    }

    public static void portalPair(PortalPair pair) {
        gs.setFill(pair.getPortal1().getColor());
        gs.fillOval(pair.getPortal1().getpX(), pair.getPortal1().getpY(), pair.getPortal1().getWitdh(), pair.getPortal1().getHeight());
        gs.setFill(pair.getPortal2().getColor());
        gs.fillOval(pair.getPortal2().getpX(), pair.getPortal2().getpY(), pair.getPortal1().getWitdh(), pair.getPortal1().getHeight());
        gs.setFill(Background);
    }
